...
ifeq ($(SOC),gxl)
	$(Q)$(FIP_FOLDER_SOC)/aml_encrypt_$(SOC) --efsgen --amluserkey $(srctree)/board/$(BOARDDIR)/aml-user-key.sig \
			--output $(FIP_FOLDER_SOC)/u-boot.bin.encrypt.efuse
endif
	$(Q)$(FIP_FOLDER_SOC)/aml_encrypt_$(SOC) --bootsig --input $(FIP_FOLDER_SOC)/u-boot.bin --amluserkey $(srctree)/board/$(BOARDDIR)/aml-user-key.sig \
